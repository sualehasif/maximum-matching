...
namespace wr {
	...
	MetaGraphsSolver<Label>::MetaGraphsSolver(unsigned int maxMetaNeighbors, unsigned int arg2)
		: vertices()
		, remainingTrees()
		, growQueue()
		, waitingRoom()
	...
	template <class Label>
	MetaGraphsSolver<Label>::MetaGraphsSolver() : MetaGraphsSolver(1000, 0) {};
	...
	void MetaGraphsSolver<Label>::addVertex(MVertex<Label> * v) {
		...
		v->setWaitingRoom(&this->waitingRoom);
	}
	...
	void MetaGraphsSolver<Label>::calculateMaxMatching() {
		...
		while (keepRunning) {
			...
			MetaGraphsSolver<Label> metaSolver(this->maxMetaNeighbors, 0);
	...
	void MetaGraphsSolver<Label>::growFrom(MVertex<Label> * v) {
		...
		/* Put vertices from well connected cherry trees aside */
		if (tv->metaVertex->neighbors.size() >= this->maxMetaNeighbors) {
			v->startWaiting();
			return;
		}
	...
	template <class Label>
	void MetaGraphsSolver<Label>::clearWaitingRoom() {
		DEBUG("Requeueing waiting room of " << this->waitingRoom.getSize() << " vertices.\n");
		while (!this->waitingRoom.isEmpty()) {
			MVertex<Label>* v = this->waitingRoom.popElem()->value;
			v->enqueue();
		}
		DEBUG("Finished requeueing waiting room\n");
	}
